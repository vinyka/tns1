...
const useStyles = makeStyles((theme) => ({
  ...
  content: ({ hasBanner }) => ({
    flex: 1,
    overflow: "auto",
    paddingBottom: hasBanner ? theme.spacing(0.2) : 0,
  }),
  container: {
  ...
  footerWarning: {
    ...
    background:
      theme.mode === "light"
        ? `linear-gradient(135deg, ${theme.palette.common.white} 0%, ${theme.palette.warning.light} 100%)`
        : `linear-gradient(135deg, ${theme.palette.background.paper} 0%, ${theme.palette.warning.dark} 100%)`,
...
const LoggedInLayout = ({ children, themeToggle }) => {
  ...
  const hasDisconnectedWhatsApp = useMemo(() => {
    ...
    return whatsApps.some((session) => offlineStatuses.includes(session.status));
  }, [whatsApps]);

  const classes = useStyles({ hasBanner: hasDisconnectedWhatsApp });
  ...
  const footerPosition = useMemo(() => {
    const gutter = greaterThenSm ? theme.spacing(4) : theme.spacing(2);

    if (!greaterThenSm || drawerVariant === "temporary") {
      return {
        left: gutter,
        right: gutter,
    ...
    const baseWidth = drawerOpen ? drawerWidth : collapsedWidth;

    return {
      left: baseWidth + gutter,
      right: gutter,
      bottom: theme.spacing(1),
    };
  }, [drawerOpen, drawerVariant, greaterThenSm, theme]);
  ...
  return (
    <div className={classes.root}>
      ...
      <main className={classes.content}>
        ...
        {hasDisconnectedWhatsApp && (
          <div className={classes.footerWarning} style={footerPosition}>
            <div className={classes.footerWarningIconWrap}>
              <WarningAmberRoundedIcon className={classes.footerWarningIcon} />
            </div>
            <div className={classes.footerWarningContent}>
              <Typography className={classes.footerWarningTitle}>
                {i18n.t("mainDrawer.footerWarning.title")}
              </Typography>
              <Typography variant="body2">
                {i18n.t("mainDrawer.footerWarning.description")}
              </Typography>
              <Typography className={classes.footerWarningHelp}>
                {i18n.t("mainDrawer.footerWarning.help")}
              </Typography>
            </div>
            <Button
              variant="contained"
              ...
              to="/connections"
              className={classes.footerWarningButton}
            >
              {i18n.t("mainDrawer.footerWarning.cta")}
            </Button>
          </div>
        )}
      </main>
