    ...
    storage: multer_1.default.diskStorage({
        destination: async function (req, file, cb) {
            ...
            if (typeArch && typeArch !== "announcements" && typeArch !== "logo") {
                folder = path_2.default.resolve(publicFolder, `company${companyId}`, typeArch, fileId ? fileId : "");
            }
            else if (typeArch && typeArch === "announcements") {
                folder = path_2.default.resolve(publicFolder, typeArch);
            }
            else if (typeArch === "logo") {
                folder = path_2.default.resolve(publicFolder);
            }
            else {
                folder = path_2.default.resolve(publicFolder, `company${companyId}`);
            }
