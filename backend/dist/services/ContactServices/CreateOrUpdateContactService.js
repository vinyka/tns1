...
const downloadProfileImage = async ({ profilePicUrl, companyId, contact }) => {
    ...
    let filename;
    const folder = path_2.default.resolve(publicFolder, `company${companyId}`, "contacts");
    ...
    try {
        ...
        fs_2.default.writeFileSync((0, path_2.join)(folder, filename), response.data);
    }
    catch (error) {
        console.error(error);
    }
    ...
    try {
        ...
        if (contact) {
            ...
            if (contact.urlPicture) {
                ...
                oldPath = path_2.default.resolve(contact.urlPicture.replace(/\\/g, '/'));
                fileName = path_2.default.join(folder, oldPath.split('\\').pop());
            }
            if (!fs_2.default.existsSync(fileName) || contact.profilePicUrl === "") {
