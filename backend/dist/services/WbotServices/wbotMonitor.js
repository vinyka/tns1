    ...
    try {
        ...
        wbot.ev.on("contacts.upsert", async (contacts) => {
            ...
            try {
                ...
                if (!fs_2.default.existsSync(path_2.default.join(publicFolder, `company${companyId}`))) {
                    fs_2.default.mkdirSync(path_2.default.join(publicFolder, `company${companyId}`), { recursive: true });
                    fs_2.default.chmodSync(path_2.default.join(publicFolder, `company${companyId}`), 0o777);
                }
                const contatcJson = path_2.default.join(publicFolder, `company${companyId}`, "contactJson.txt");
                if (fs_2.default.existsSync(contatcJson)) {
                    await fs_2.default.unlinkSync(contatcJson);
                }
                await fs_2.default.promises.writeFile(contatcJson, JSON.stringify(filteredContacts, null, 2));
            }
            catch (err) {
