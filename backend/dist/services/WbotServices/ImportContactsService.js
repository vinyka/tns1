...
const ImportContactsService = async (companyId, whatsappId) => {
    ...
    try {
        ...
        const beforeFilePath = path_2.default.join(publicFolder, `company${companyId}`, 'contatos_antes.txt');
        fs_2.default.writeFile(beforeFilePath, JSON.stringify(phoneContacts, null, 2), (err) => {
            if (err) {
    ...
    const publicFolder = path_2.default.resolve(__dirname, "..", "..", "..", "public");
    const afterFilePath = path_2.default.join(publicFolder, `company${companyId}`, 'contatos_depois.txt');
    fs_2.default.writeFile(afterFilePath, JSON.stringify(phoneContacts, null, 2), (err) => {
        if (err) {
            logger_1.default.error(`Failed to write contacts to file: ${err}`);
            throw err;
        }
