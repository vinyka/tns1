...
const writeFileAsync = (0, util_1.promisify)(fs_2.writeFile);
function removeFile(directory) {
    fs_3.default.unlink(directory, (error) => {
        if (error)
            throw error;
    });
}
...
const verifyMediaMessage = async (msg, ticket, contact, ticketTraking, isForwarded = false, isPrivate = false, wbot) => {
    ...
    try {
        ...
        try {
            const folder = path_2.default.resolve(__dirname, "..", "..", "..", "public", `company${companyId}`);
            // const folder = `public/company${companyId}`; // Correção adicionada por Altemir 16-08-2023
            if (!fs_3.default.existsSync(folder)) {
                fs_3.default.mkdirSync(folder, { recursive: true }); // Correção adicionada por Altemir 16-08-2023
                fs_3.default.chmodSync(folder, 0o777);
            }
            await writeFileAsync((0, path_2.join)(folder, media.filename), media.data.toString('base64'), "base64") // Correção adicionada por Altemir 16-08-2023
                .then(() => {
                // console.log("Arquivo salvo com sucesso!");
                if (media.mimetype.includes("audio")) {
                    console.log(media.mimetype);
                    const inputFile = path_2.default.join(folder, media.filename);
                    let outputFile;
    ...
    if (queues.length === 1 && !chatbot) {
        ...
        if (greetingMessage.length > 1 && sendGreetingMessageOneQueues) {
            ...
            if (ticket.whatsapp.greetingMediaAttachment !== null) {
                const filePath = path_2.default.resolve("public", `company${companyId}`, ticket.whatsapp.greetingMediaAttachment);
                const fileExists = fs_3.default.existsSync(filePath);
                if (fileExists) {
        ...
        if (!(0, lodash_1.isNil)(queues[0].fileListId)) {
            ...
            try {
                ...
                const folder = path_2.default.resolve(publicFolder, `company${ticket.companyId}`, "fileList", String(files.id));
                for (const [index, file] of files.options.entries()) {
                    const mediaSrc = {
                        ...
                        filename: file.path,
                        path: path_2.default.resolve(folder, file.path),
                    };
...
const handleMessageIntegration = async (msg, wbot, companyId, queueIntegration, ticket, isMenu, whatsapp, contact, isFirstMsg) => {
    ...
    if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
        ...
        if (msgType === "audioMessage") {
            ...
            (0, fs_2.readFile)((0, path_2.join)(__dirname, "..", "..", "..", "public", `company${companyId}`, filename), "base64", (err, data) => {
                inputAudio = data;
                if (err) {
                    logger_1.default.error(err);
                }
...
const filterMessages = (msg) => {
    ...
    if (msg.messageStubType && blockedStubTypes.includes(msg.messageStubType))
        return false;
    return true;
};
